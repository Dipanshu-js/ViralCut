  ...
  async generate(req: GenerationRequest): Promise<GenerationResult> {
    ...
    if (!apiKey)
      return {
        ...
        provider: this.name,
        estimatedTime: 0,
        message: "Add PEXELS_API_KEY (free at pexels.com/api)",
      };

    // Extract keywords from prompt for search
    const keywords = this.extractKeywords(
      req.prompt || req.enhancedPrompt || "",
    );
    ...
    try {
      ...
      if (!video)
        return {
          ...
          provider: this.name,
          estimatedTime: 0,
          message: `No Pexels video found for: ${keywords}`,
        };
  ...
  async generate(req: GenerationRequest): Promise<GenerationResult> {
    ...
    if (!apiKey)
      return {
        ...
        provider: this.name,
        estimatedTime: 0,
        message: "Add PIXABAY_API_KEY (free at pixabay.com/api)",
      };
    ...
    try {
      ...
      if (!hit)
        return {
          ...
          provider: this.name,
          estimatedTime: 0,
          message: `No Pixabay video for: ${keywords}`,
        };
...
export class CanvasSlideProvider implements VideoProvider {
  ...
  async generate(req: GenerationRequest): Promise<GenerationResult> {
    ...
    const animations: Array<"zoom" | "slide-up" | "fade" | "pan"> = [
      "zoom",
      "slide-up",
      "fade",
      "pan",
    ...
    const scenes = sentences
      .slice(0, Math.min(4, Math.ceil(req.duration / 2)))
      .map((sentence, i) => ({
...
export class GrokVideoProvider implements VideoProvider {
  ...
  getEstimatedTime(duration: number) {
  ...
  async generate(req: GenerationRequest): Promise<GenerationResult> {
    ...
    if (!apiKey)
      return {
        ...
        provider: this.name,
        estimatedTime: 0,
        message: "Add XAI_API_KEY for Grok video",
      };
    ...
    try {
      ...
      if (!res.ok)
        return {
          ...
          provider: this.name,
          estimatedTime: 0,
          message: `Grok API error: ${res.status}`,
        };
      const data = (await res.json()) as {
        video_url?: string;
        url?: string;
      ...
      return {
        ...
        provider: this.name,
        estimatedTime: this.getEstimatedTime(req.duration),
      };
    } catch (e) {
      return {
        videoUrl: null,
  ...
  async generate(req: GenerationRequest): Promise<GenerationResult> {
    if (!(await this.isAvailable())) {
      return {
        ...
        provider: this.name,
        estimatedTime: 0,
        message: "ComfyUI not running on localhost:8188",
      };
    }
    ...
    try {
      ...
      if (!submitRes.ok)
        return {
          ...
          provider: this.name,
          estimatedTime: this.getEstimatedTime(req.duration),
          message: "ComfyUI submission failed",
        };
      ...
      for (let i = 0; i < maxPolls; i++) {
        ...
        const histRes = await fetch(`${this.comfyUrl}/history/${promptId}`, {
  ...
  private buildComfyWorkflow(req: GenerationRequest): Record<string, unknown> {
    ...
    return {
      "1": {
        ...
        inputs: {
          clip: ["4", 1],
          text: `${req.style} style, ${req.enhancedPrompt || req.prompt}, cinematic, high quality, 8K`,
        },
      },
      "2": {
        class_type: "CLIPTextEncode",
        inputs: {
          clip: ["4", 1],
          text: "blurry, low quality, distorted, watermark, text, ugly, deformed",
        },
      },
      "3": {
        class_type: "KSampler",
      ...
      "12": {
        ...
        inputs: {
          context_length: Math.min(frames, 16),
          context_stride: 1,
          context_overlap: 4,
    ...
    try {
      const res = await fetch(
        "https://api.dev.runwayml.com/v1/image_to_video",
        {
          ...
          body: JSON.stringify({
            promptText: req.enhancedPrompt || req.prompt,
            model: "gen3a_turbo",
      ...
      return {
        ...
        jobId: data.id,
        status: data.output ? "done" : "pending",
        provider: this.name,
        estimatedTime: this.getEstimatedTime(req.duration),
      };
    } catch (e) {
  ...
  async generate(req: GenerationRequest): Promise<GenerationResult> {
    ...
    try {
      ...
      return {
        ...
        jobId: data.id,
        status: data.video ? "done" : "pending",
        provider: this.name,
        estimatedTime: this.getEstimatedTime(req.duration),
      };
    } catch (e) {
  ...
  async generate(req: GenerationRequest): Promise<GenerationResult> {
    ...
    try {
      ...
      return {
        ...
        jobId: data.id,
        status: data.assets?.video ? "done" : "pending",
        provider: this.name,
        estimatedTime: this.getEstimatedTime(req.duration),
      };
    } catch (e) {
...
export async function checkProviderAvailability(): Promise<{
  ...
  const freeMap: Record<string, { label: string; note: string }> = {
    pexels: {
      ...
      note: "Free API key at pexels.com/api",
    },
    pixabay: {
      label: "Pixabay Stock Video",
      note: "Free API key at pixabay.com/api",
    },
    canvas: {
      label: "Canvas Slideshow",
      note: "Always free — no setup required",
    },
    local: {
      label: "Local AI (ComfyUI)",
      note: "Requires GPU + ComfyUI install",
    },
  };
